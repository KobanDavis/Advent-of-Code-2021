...
const addLineDiagonal = (x1: number, y1: number, x2: number, y2: number) => {
	const diffX = x1 - x2
	const diffXIsNegative = x1 < x2
	const diffYIsNegative = y1 < y2

	const points = []

	for (let i = 0; i < Math.abs(diffX) + 1; i++) {
		if (diffXIsNegative) {
			if (diffYIsNegative) {
